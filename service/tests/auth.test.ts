...
async function token(options: {
  ...
  const jwt = new SignJWT(options.role === null ? {} : { role: options.role ?? "authenticated" })
    ...
    .setIssuedAt();
  if (options.subject !== null) jwt.setSubject(options.subject ?? USER_ID);
  if (options.expiration !== null) jwt.setExpirationTime(options.expiration ?? Math.floor(Date.now() / 1000) + 300);
...
describe("Supabase access-token verification", () => {
  ...
  it("rejects a token signed by a key outside the project JWKS", async () => {
    ...
    await expect(createTokenVerifier(supabaseUrl)(await token({ key: foreignKey }))).rejects.toBeInstanceOf(
  ...
  it("rejects a legacy shared-secret signature", async () => {
    ...
    await expect(createTokenVerifier(supabaseUrl)(legacyToken)).rejects.toBeInstanceOf(
      UnauthenticatedError,
    );
  });
  ...
  it("rejects an expired token", async () => {
    await expect(
      createTokenVerifier(supabaseUrl)(await token({ expiration: Math.floor(Date.now() / 1000) - 1 })),
    ).rejects.toBeInstanceOf(UnauthenticatedError);
  });

  it("rejects a token issued by another project", async () => {
    await expect(
      createTokenVerifier(supabaseUrl)(await token({ issuer: "https://other.supabase.co/auth/v1" })),
    ).rejects.toBeInstanceOf(UnauthenticatedError);
  });

  it("rejects a token for the wrong audience", async () => {
    await expect(
      createTokenVerifier(supabaseUrl)(await token({ audience: "anon" })),
    ).rejects.toBeInstanceOf(UnauthenticatedError);
  });
  ...
  ] as const)("rejects a token without the required %s claim", async (_name, options) => {
    await expect(createTokenVerifier(supabaseUrl)(await token(options))).rejects.toBeInstanceOf(
      UnauthenticatedError,
    );
  });
  ...
  it("rejects a non-user subject", async () => {
    await expect(
      createTokenVerifier(supabaseUrl)(await token({ subject: "not-a-user-uuid" })),
    ).rejects.toBeInstanceOf(UnauthenticatedError);
  });

  it("rejects a non-authenticated Postgres role", async () => {
    await expect(
      createTokenVerifier(supabaseUrl)(await token({ role: "service_role" })),
    ).rejects.toBeInstanceOf(UnauthenticatedError);
  });
});
