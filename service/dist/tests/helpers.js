...
import { TILE_TOKENS } from "../src/canonical.js";
...
export function buildInventory(options = {}) {
    const inventory = new Array(TILE_TOKENS.length);
    const taken = new Set();
    const claimOrdinal = (token) => {
    ...
    let bagSeq = 0;
    for (let ordinal = 0; ordinal < TILE_TOKENS.length; ordinal += 1) {
        if (!inventory[ordinal])
            inventory[ordinal] = { at: "bag", seq: bagSeq++ };
    }
    return inventory;
}
...
export function fakeEngineResponse(overrides = {}) {
    return {
        type: "place",
        placements: [
