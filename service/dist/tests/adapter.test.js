...
describe("canonical parsing", () => {
    ...
    it("refuses two tiles on one square rather than picking a winner", () => {
        ...
        expect(() => parseCanonical({ ...buildCanonicalPayload(), inventory })).toThrow(/Two tiles occupy square/);
    });
    it("refuses a tile off the board", () => {
        ...
        expect(() => parseCanonical({ ...buildCanonicalPayload(), inventory })).toThrow(/outside the board/);
    });
});
...
describe("exchange rule", () => {
    const withCounts = (rackB, bagKeep) => {
        const payload = buildCanonicalPayload({
            activeSide: "A",
            rackA: ["1", "2", "3", "4", "5", "6", "7", "8"],
            rackB: Array.from({ length: rackB }, () => "="),
        });
