...
suite("the real engine", () => {
    ...
    it("only ever plays tiles that are on the analysed rack", async () => {
        ...
        for (const candidate of response.candidates ?? []) {
            ...
            for (const kind of used) {
                ...
                expect(index, `candidate uses "${kind}" which is not (or no longer) on the rack`).toBeGreaterThanOrEqual(0);
    ...
    it("produces the same result twice for the same position, seed and level", async () => {
        ...
        expect(second.candidates?.map((candidate) => candidate.value)).toEqual(first.candidates?.map((candidate) => candidate.value));
    }, 240_000);
    ...
    it("kills a search that overruns its wall-clock ceiling", async () => {
        ...
        await expect(runEngine({ binaryPath: ENGINE, request, timeoutMs: 1_500 })).rejects.toBeInstanceOf(EngineTimeoutError);
    }, 60_000);
