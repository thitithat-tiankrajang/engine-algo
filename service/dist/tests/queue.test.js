...
describe("concurrency", () => {
    it("never runs more searches at once than it is allowed to", async () => {
        ...
        const jobs = gates.map((gate, index) => queue.submit({
            key: `job-${index}`,
            priority: 0,
            run: async () => {
                running += 1;
...
describe("priority", () => {
    ...
    it("keeps arrival order within one priority", async () => {
        ...
        await tick();
        const jobs = [1, 2, 3].map((index) => queue.submit({
            key: `job-${index}`,
            priority: 20,
            run: async () => {
                order.push(index);
...
describe("admission", () => {
    it("refuses work rather than accepting it into a queue it cannot serve", async () => {
        ...
        const blocker = deferred();
        const accepted = [0, 1, 2].map((index) => queue.submit({
            key: `job-${index}`,
            priority: 0,
            run: async () => {
                await blocker.promise;
        ...
        await expect(queue.submit({ key: "overflow", priority: 0, run: async () => undefined })).rejects.toBeInstanceOf(QueueFullError);
        blocker.resolve();
