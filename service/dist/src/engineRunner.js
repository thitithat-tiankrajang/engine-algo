...
export async function runEngine(options) {
    ...
    try {
        ...
        await stopProcess();
        throw new EngineFailureError("The engine process could not be given its request.", error instanceof Error ? error.message : undefined);
    }
    let closed;
    try {
        closed = await exit;
        ...
        throw new EngineFailureError("The engine process could not be started.", error instanceof Error ? error.message : undefined);
    }
    finally {
        clearTimeout(timer);
    ...
    if (closed.code !== 0) {
        throw new EngineFailureError(`The engine exited with ${closed.signal ?? `code ${closed.code}`}.`, stderrTail || undefined);
    }
    let parsed;
    try {
