...
import { EXCHANGE_MIN_RESERVE, RACK_SIZE, bagSize, boardCells, otherSide, pendingReturnSize, rackSize, rackTokens, } from "./canonical.js";
/**
 ...
 */
export function exchangeAllowed(state) {
    const bag = bagSize(state);
    if (state.gameMode === "solo")
        return bag >= EXCHANGE_MIN_RESERVE;
 ...
 */
export function noScoreStreak(commands) {
    let streak = 0;
 ...
 */
export function seedFor(gameId, revision, salt = "") {
 ...
 */
export function toEngineRequest(state, options) {
    const side = options.side;
    const opponent = otherSide(side);
    const rack = rackTokens(state, side);
    return {
