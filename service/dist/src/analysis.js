...
function describeMove(candidate) {
...
function factorsFor(candidate, solver) {
    const factors = [];
    if (solver === "endgame") {
 ...
 */
function noteFor(candidate, best, solver) {
    if (candidate === best) {
        if (solver === "endgame") {
...
function summaryFor(best, runnerUp, response, complete) {
    const parts = [];
    const action = describeMove(best);
    if (response.solver === "endgame") {
        const margin = round(best.value);
        parts.push(`The bag is empty enough to solve exactly, so this is not an estimate: ${action} ` +
            (margin > 0
    ...
    if (response.solver === "greedy") {
        parts.push(`With no opponent rack to reason about, the engine ranked moves on immediate value alone and chose ${action}.`);
    }
    else {
        parts.push(`Across ${response.stats.samples} simulated opponent racks, ${action} came out best.`);
    }
    ...
    if (response.solver === "sim" && best.stddev > 0.05) {
        parts.push(`Its value varies by about ${round(best.stddev)} depending on what the opponent actually holds, ` +
    ...
    if (!complete) {
        parts.push("The search hit its time limit before finishing every scenario, so this ranking is provisional.");
    }
    return parts.join(" ");
}
export class AnalysisUnavailableError extends Error {
    name = "AnalysisUnavailableError";
}
export function buildAnalysis(input) {
    ...
    if (rows.length === 0) {
        ...
        throw new AnalysisUnavailableError("The engine found no candidate moves to compare in this position.");
    }
    ...
    const complete = response.solver !== "sim" || response.stats.samples >= input.requestedSamples;
    const toCandidate = (row, index) => {
        ...
        if (row !== best) {
            ...
            for (const factor of factors) {
                ...
                if (counterpart)
                    factor.delta = round(factor.value - counterpart.value);
            }
        }
        return {
            rank: index + 1,
