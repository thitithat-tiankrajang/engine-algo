...
const TOKEN_COUNTS: Record<AmathToken, number> = {
...
export const TILE_TOKENS: readonly AmathToken[] = (() => {
  const tokens: AmathToken[] = [];
  for (const token of TOKEN_ORDER) {
    for (let copy = 0; copy < TOKEN_COUNTS[token]; copy += 1) tokens.push(token);
  }
...
if (TILE_TOKENS.length !== TILE_COUNT) {
  throw new Error(
    `The tile manifest describes ${TILE_TOKENS.length} tiles but the physical set has ${TILE_COUNT}.`,
  );
}
...
export type TilePlacement =
  ...
  | { at: "board"; row: number; col: number; placedTurn: number; by: Side; assigned?: string };
 ...
 */
export function parseCanonical(payload: unknown): CanonicalState {
  ...
  if (inventory.length !== TILE_COUNT) {
    throw new CanonicalStateError(
      `Canonical state describes ${inventory.length} tiles, not the ${TILE_COUNT}-tile set.`,
    );
  }
  ...
  for (let ordinal = 0; ordinal < TILE_COUNT; ordinal += 1) {
    const entry = inventory[ordinal] as Record<string, unknown> | undefined;
...
export function tokenOfOrdinal(ordinal: number): AmathToken {
...
export function rackTokens(state: CanonicalState, side: Side): AmathToken[] {
  const held: Array<{ seq: number; token: AmathToken }> = [];
...
export function rackSize(state: CanonicalState, side: Side): number {
  let total = 0;
  for (const placement of state.inventory) {
    if (placement.at === "rack" && placement.side === side) total += 1;
  }
  return total;
}

export function bagSize(state: CanonicalState): number {
  let total = 0;
  for (const placement of state.inventory) {
    if (placement.at === "bag") total += 1;
  }
  return total;
}

export function pendingReturnSize(state: CanonicalState): number {
  let total = 0;
  for (const placement of state.inventory) {
    if (placement.at === "pendingReturn") total += 1;
  }
  return total;
}

export type BoardCell = { r: number; c: number; kind: AmathToken; token: string };
...
export function boardCells(state: CanonicalState): BoardCell[] {
  const cells: BoardCell[] = [];
  state.inventory.forEach((placement, ordinal) => {
    if (placement.at !== "board") return;
